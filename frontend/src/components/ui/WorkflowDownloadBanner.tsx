  ...
  if (isDownloaderNode && liveFiles.length > 0) {
    ...
    return (
      <div className="border-b border-amber-500/20 bg-amber-500/[0.06] px-5 py-3 space-y-2">
          ...
          {liveFiles.map((f) => (
            <div key={`${f.folder}/${f.filename}`} className="space-y-1">
              <div className="flex items-center justify-between gap-3">
                <span className="text-[10px] text-fedda-text-2 truncate min-w-0 font-mono">{f.filename}</span>
                <span className="text-[10px] font-mono text-fedda-text-4 flex-shrink-0">
                  {f.exists ? '✓ Done' : f.currentBytes > 0 ? fmtBytes(f.currentBytes) : 'Waiting…'}
                </span>
              </div>
        ...
        <p className="text-[10px] text-fedda-text-4">Runs automatically when downloads complete.</p>
      </div>
    );
  }
    ...
    return (
      <div className="border-b border-amber-500/15 bg-amber-500/[0.04] px-5 py-2.5 space-y-1.5">
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-0.5 pl-5">
          {missing.map((f) => (
            <span key={`${f.folder}/${f.filename}`} className="text-[10px] font-mono text-fedda-text-4 truncate">
              {f.filename}
            </span>
          ))}
        </div>
        <p className="text-[10px] text-fedda-text-4 pl-5">Click Generate — downloads and runs automatically.</p>
      </div>
    );
  }
