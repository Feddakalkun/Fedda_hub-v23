...
const CharacterCard = ({
    ...
    return (
        <div className="group relative aspect-[3/4] rounded-xl overflow-hidden border border-white/[0.06] bg-fedda-bg-2">
            {item.preview_url && !imgFailed ? (
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                    <ImageOff className="w-8 h-8 text-fedda-text-4" />
                </div>
            )}
                ...
                <div className="mt-2">
                    {item.installed ? (
                        ...
                        <button
                            ...
                            className="px-2.5 py-1 bg-white/10 hover:bg-white hover:text-black text-white rounded-lg text-[9px] font-bold uppercase tracking-widest transition-all disabled:opacity-40"
                        >
...
export const LoRADownloader = ({ family = 'z-image' }: LoRADownloaderProps) => {
    ...
    if (isLoading) return (
        <div className="flex items-center justify-center h-40">
            <Loader2 className="w-5 h-5 text-fedda-accent animate-spin" />
        </div>
    );
    ...
    return (
        <CatalogShell title={title} subtitle="Download character & style LoRAs for use in generation." icon={Users}>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <CatalogCard
                    ...
                    icon={Upload}
                    iconClassName={uploadingLocal ? 'animate-spin text-fedda-accent' : ''}
                    actionLabel={uploadingLocal ? 'Uploading...' : 'Choose File'}
                    onAction={uploadingLocal ? undefined : () => fileInputRef.current?.click()}
                    className={isDropTarget ? 'ring-2 ring-fedda-accent border-fedda-accent/50' : ''}
                >
                    <input
                        ref={fileInputRef}
                        type="file"
                        accept=".safetensors,.ckpt"
                        onChange={e => { const f = e.target.files?.[0]; if (f) handleLocalFileUpload(f); e.target.value = ''; }}
                        className="hidden"
                    />
                    <div
                        onDrop={handleDrop}
                        onDragOver={e => { e.preventDefault(); setIsDropTarget(true); }}
                        onDragLeave={e => { e.preventDefault(); setIsDropTarget(false); }}
                ...
                {isZImage && (
                    <CatalogCard
                        ...
                        iconClassName={anyStarterLoading ? 'animate-spin text-fedda-accent' : allStarterInstalled ? 'text-emerald-400' : ''}
                ...
                {packs.map(pack => {
                    ...
                    return (
                        <CatalogCard
                            key={pack.key}
                            title={pack.title}
                            subtitle={cat.total ? `${cat.installed}/${cat.total} installed` : 'Loading catalog...'}
                            icon={busy ? Loader2 : Users}
                            iconClassName={busy ? 'animate-spin text-fedda-accent' : ''}
                ...
                {/* Manual import */}
                <div className="rounded-xl border border-white/[0.06] bg-fedda-bg-1 p-6 flex flex-col justify-between">
                    <div className="space-y-3">
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-xl bg-fedda-bg-2 border border-white/[0.08] flex items-center justify-center">
                                <DownloadCloud className="w-5 h-5 text-fedda-text-3" />
                            </div>
                            <div className="text-sm font-semibold text-fedda-text-1">Import from URL</div>
                        </div>
                        <input
                            value={importUrl}
                            ...
                            className="w-full rounded-xl border border-white/10 bg-fedda-bg-2 px-3 py-2 text-sm text-fedda-text-1 outline-none focus:border-white/20 placeholder:text-fedda-text-4"
                        />
                        {importStatus && (
                            <p className="text-[11px] text-fedda-text-3">{importStatus}</p>
                        )}
                    </div>
                    <button
                        onClick={handleImportUrl}
                        className="mt-4 w-full py-2.5 bg-fedda-bg-2 hover:bg-fedda-bg-3 border border-white/[0.06] rounded-xl text-[11px] font-semibold text-fedda-text-2 transition-all"
                    >
            ...
            {previewOpen && (
                <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="w-full max-w-6xl h-full max-h-[90vh] bg-fedda-bg-1 border border-white/[0.08] rounded-xl overflow-hidden flex flex-col shadow-2xl">

                        <div className="px-6 py-4 border-b border-white/[0.06] flex items-center gap-4 shrink-0">
                            <div className="flex-1 min-w-0">
                                <h3 className="text-base font-semibold text-fedda-text-1 truncate">{previewTitle}</h3>
                                <p className="text-[11px] text-fedda-text-3 mt-0.5">
                            ...
                            <input
                                ...
                                className="w-48 rounded-xl border border-white/10 bg-fedda-bg-2 px-3 py-1.5 text-sm text-fedda-text-1 outline-none focus:border-white/20 placeholder:text-fedda-text-4"
                            />
                            {activePreviewPack && (
                                <button
                                    onClick={() => handleSyncPack(activePreviewPack)}
                                    className="px-4 py-2 bg-fedda-accent hover:bg-fedda-accent/90 text-white rounded-xl text-[11px] font-semibold transition-all shrink-0"
                                >
                                    Download All
                                </button>
                            )}
                            <button
                                ...
                                className="w-8 h-8 rounded-xl bg-fedda-bg-2 hover:bg-fedda-bg-3 flex items-center justify-center border border-white/[0.06] transition-all text-fedda-text-3 hover:text-fedda-text-1 shrink-0"
                            >
                                <X className="w-4 h-4" />
                            </button>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-6">
                            {filteredPreview.length === 0 ? (
                                <div className="flex items-center justify-center h-40 text-fedda-text-3 text-sm">No results</div>
                            ) : (
