...
export const mediaSourceIsVideo = (source: string, url: string): boolean => {
  ...
  return (
    s.includes('video') || s.includes('vid') || s.includes('wan') || s.includes('ltx') ||
    u.includes('.mp4') || u.includes('.webm') || u.includes('.mov')
  );
};
...
export const loadStoredMedia = (maxItems = DEFAULT_MAX_ITEMS): MediaItem[] => {
  ...
  keys.forEach((key) => {
    ...
    urls.forEach((url) => {
      ...
      seen.add(url);
      merged.push({ url, source, kind: mediaSourceIsVideo(source, url) ? 'video' : 'image' });
    });
  });
