...
export const useOllamaManager = () => {
  ...
  useEffect(() => { setSelectedModel(activeList[0].id); }, [modelCategory]);
  ...
  const refreshModels = async () => {
    ...
    try {
      ...
      // silent — status indicator handles offline state
    } finally {
  ...
  const handlePull = async () => {
    ...
    try {
      await ollamaService.pullModel(modelToPull, (progress) => {
        ...
        if (progress.status === 'success') setTimeout(refreshModels, 1000);
      });
      setCustomModel('');
    } catch {
  ...
  return {
    ...
    modelCategory, setModelCategory, activeList,
    selectedModel, setSelectedModel,
    customModel, setCustomModel,
    isPulling, pullProgress, pullError,
    refreshModels, handlePull, handleDelete,
  };
};
